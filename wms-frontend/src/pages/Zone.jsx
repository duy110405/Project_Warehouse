...
import { Modal, Form, Input, InputNumber, Select, ConfigProvider, theme, message, Spin, Empty, Popconfirm, Button } from 'antd';
...
const ZoneGrid = ({ zones, isLoading, onEdit, onDelete, activeTab }) => {
  ...
  if (!zones || zones.length === 0) return <div className="bg-[#0F172A] rounded-2xl py-20 border border-slate-800"><Empty description={<span className="text-slate-400">Chưa có khu vực nào</span>} /></div>;
          ...
          <div key={zone.zoneId || zone.id} className="bg-[#151E32] border border-slate-700 hover:border-slate-600 transition-colors rounded-xl p-5">
            <div className="flex justify-between items-start mb-5">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold ${activeTab === 1 ? 'bg-blue-500/20 text-blue-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
              ...
              <div className="flex gap-2">
                <button onClick={() => onEdit(zone)} className="p-2 text-slate-400 hover:text-blue-400 bg-[#1E293B] hover:bg-blue-400/10 rounded-lg transition-colors border border-slate-700 hover:border-blue-500/50">
                  <Edit size={16} />
                </button>
                <Popconfirm title="Xóa khu vực" description={`Bạn có chắc muốn xóa "${zone.zoneName}"?`} onConfirm={() => onDelete(zone.zoneId || zone.id)} okText="Xóa" cancelText="Hủy" okButtonProps={{ danger: true }}>
...
const ZoneModal = ({ isOpen, onClose, form, onSubmit, isEditing }) => (
  ...
  >
    <Form form={form} layout="vertical" onFinish={onSubmit} className="mt-6">
      ...
      <Form.Item name="currentLoad" hidden>
        <InputNumber />
      </Form.Item>

      <Form.Item name="zoneId" label={<span className="text-slate-300">Mã khu vực</span>}>
        <Input placeholder="Hệ thống tự động tạo..." className="bg-[#1E293B] border-slate-700 text-slate-500 py-2" disabled />
      </Form.Item>

      <Form.Item name="zoneName" label={<span className="text-slate-300">Tên khu vực</span>} rules={[{ required: true, message: 'Vui lòng nhập tên khu vực!' }]}>
        <Input placeholder="VD: Khu A - Hóa chất" className="bg-[#1E293B] border-slate-700 text-white py-2" />
      </Form.Item>

      <Form.Item name="zoneType" label={<span className="text-slate-300">Loại khu vực (Dành cho)</span>} rules={[{ required: true, message: 'Vui lòng chọn loại khu vực!' }]}>
        <Select className="h-[40px] custom-dark-select">
          <Select.Option value={1}>Kho Nguyên Liệu</Select.Option>
          <Select.Option value={2}>Kho Thành Phẩm</Select.Option>
        </Select>
      </Form.Item>

      <Form.Item name="capacity" label={<span className="text-slate-300">Sức chứa tối đa (Capacity)</span>} rules={[{ required: true, message: 'Vui lòng nhập sức chứa!' }]}>
        <InputNumber className="w-full bg-[#1E293B] border-slate-700 text-white h-[40px]" formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} parser={(value) => value?.replace(/\$\s?|(,*)/g, '')} min={1} />
      </Form.Item>
...
export default function Zone() {
  ...
  const handleOpenModal = (zone = null) => {
    if (zone) {
      ...
      setEditingId(null);
      form.resetFields();
      form.setFieldsValue({ zoneType: activeTab }); 
    }
    setIsModalOpen(true);
  };

  // SỬA: Đã thêm hàm CloseModal chuẩn để Reset Fields khi bấm Hủy
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      handleCloseModal(); // SỬA: Đóng Modal chuẩn
      fetchZones(); 
    } catch (error) {
      ...
      <div className="flex flex-col h-full max-w-[1400px] mx-auto">
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded-2xl p-6">
          ...
          <div className="flex bg-[#1E293B] p-1 rounded-xl w-fit border border-slate-700">
            <button onClick={() => setActiveTab(1)} className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 1 ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}>
              <Layers size={16} /> Khu Nguyên Liệu
            </button>
            <button onClick={() => setActiveTab(2)} className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 2 ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}>
              <Box size={16} /> Khu Thành Phẩm
            </button>
          </div>

          <ZoneGrid zones={filteredZones} isLoading={isLoading} onEdit={handleOpenModal} onDelete={handleDeleteZone} activeTab={activeTab} />
        </div>

        <ZoneModal isOpen={isModalOpen} onClose={handleCloseModal} form={form} onSubmit={handleSubmit} isEditing={!!editingId} />     
      </div>
    </ConfigProvider>
  );
}