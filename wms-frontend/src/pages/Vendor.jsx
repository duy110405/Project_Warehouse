...
import { Modal, Form, Input, Button, Popconfirm , ConfigProvider , theme } from 'antd';
...
export default function Vendor() {
    ...
    const [vendors , setVendors] = useState([
        {
            ...
            phone: '123-456-7890',
            email: 'john.doe@vendorA.com',
            address: '123 Khu công nghiệp Vsip',
            type: 'KeyBoard',
            status: 'Active',
            activeOrders: 12,
            onTimeRate: '98%',
        ...
        },
        {
            id: 2,
            code:'NCC002',
            name: 'Công ty điện tử XYZ',
            phone: '098-765-4321',
            email: 'jane.smith@vendorB.com',
            address: '456 Khu công nghiệp Vsip',
            type: 'RAM',
            status: 'Active',
            activeOrders: 8,
            onTimeRate: '95%',
  ...
  const handleOpenModal = (vendor = null) =>{
   ...
   const handleSubmit = (values) => {
    if (editingId) {
      const updatedList = vendors.map(vendor => vendor.id === editingId ? { ...vendor, ...values } : vendor);
  ...
  const handleDelete = (id) => {
    const filteredList = vendors.filter(vendor => vendor.id !== id);
    ...
    >
      <div className="flex flex-col h-full max-w-[1400px] mx-auto text-slate-200">
       ...
       <div className="flex items-center justify-between mb-8">
         <div>
          <h2 className="text-2xl font-bold text-white tracking-wide">Vendors & Suppliers</h2>
          <p className="text-slate-400 text-sm mt-1">Quản lý đối tác cung cấp nguyên liệu và xưởng gia công</p>
         </div>
       ...
       {/* 2. TOP STATS (4 Ô THỐNG KÊ TỔNG QUAN) */}
        ...
        {vendors.map((vendor) => (
          <div key={vendor.id} className="bg-[#0F172A] border border-slate-800 rounded-2xl p-6 shadow-xl hover:border-slate-600 transition-colors flex flex-col">
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              {/* Nút Edit hoặc xóa */}
             <div className='gap-3'>
               <button onClick={() => handleOpenModal(vendor)} className="p-2 text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-colors">
               <Edit size={18} />
               </button>
              <Popconfirm title="Xóa sản phẩm" description={`Bạn có chắc chắn muốn xóa "${vendor.name}"?`} onConfirm={() => handleDelete(vendor.id)} okText="Xóa" cancelText="Hủy" okButtonProps={{ danger: true }}>
         ...
         {/* MODAL THÊM / SỬA */}
       <Modal title={<span className="text-lg">{editingId ? 'Sửa thông tin nhà cung cấp' : 'Thêm nhà cung cấp mới'}</span>} open={isModalOpen} onCancel={handleCloseModal} footer={null} className="dark-modal" width={650}>
        <Form form={form} layout="vertical" onFinish={handleSubmit} className="mt-6 ">
          
          <div className="grid grid-cols-2 gap-4">
             <Form.Item name="code" label={<span className="text-slate-300">Mã nhà cung cấp</span>} rules={[{ required: true }]}>
               <Input placeholder="VD: NCC001" className="bg-[#1E293B] border-slate-700 text-white py-2" disabled={editingId !== null} />
             </Form.Item>
